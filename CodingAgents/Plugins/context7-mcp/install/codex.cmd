...
REM ============================================================
REM  context7-mcp -> Codex  (install hook)
REM  ------------------------------------------------------------
REM  Codex CLI reads MCP server registrations from
REM      %USERPROFILE%\.codex\config.toml
REM  under [mcp_servers.<name>]. We add a context7 block via
...
set "MARKER=%MARKER_DIR%\context7-mcp.Codex.installed"
set "CONFIG=%USERPROFILE%\.codex\config.toml"

if exist "%MARKER%" (
    echo   context7-mcp already registered with Codex ^(marker present^).
    exit /b 0
)
...
if errorlevel 1 (
    ...
    echo   but Codex will not be able to launch context7 until
