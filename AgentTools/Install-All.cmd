...
REM ============================================================
REM  Install-All  --  install one, several, or every agent tool
REM  ------------------------------------------------------------
REM  Sibling of CodingAgents\Install-All.cmd and
REM  AgentTerminals\Install-All.cmd, but for the agent TOOLS
REM  catalogue: TUI utilities that are useful ALONGSIDE the coding
REM  agents without being agents themselves and without hosting
REM  them (that is what AgentTerminals is for).
REM
REM  Usage:
REM    Install-All.cmd                 interactive menu
REM    Install-All.cmd all             install every tool
REM    Install-All.cmd Superfile       install just those named
...
REM  Master tool catalogue. Add new tools here (a folder NAME\ with
REM  NAME--install.cmd + NAME--is-installed.cmd).
...
if /I "%~1"=="all" (set "SELECTED=%ALL_TOOLS%" & shift & goto :parse_args)
...
:usage
echo Usage: %~nx0 [--status] [tool ...] ^| all ^| /?
echo.
echo Available tools (case-insensitive):
for %%T in (%ALL_TOOLS%) do echo   %%T
echo.
echo Examples:
echo   %~nx0                 interactive menu
echo   %~nx0 --status        print install-status table and exit
echo   %~nx0 all             install every tool
echo   %~nx0 Superfile       install just superfile
exit /b 0
...
cls
echo ============================================================
echo  Install agent tools  --  choose what to install
echo ============================================================
echo.
set "IDX=0"
...
if /I "!INPUT!"=="A" (set "SELECTED=%ALL_TOOLS%" & goto :run)
...
if not defined SELECTED (
    echo.
    echo No valid tools selected. Press any key to try again.
    pause >nul
    goto :menu
)
goto :run
...
cls
echo ============================================================
echo  Install status  --  every tool in the catalogue
echo ============================================================
echo.
set "OK=0"
set "MISS=0"
